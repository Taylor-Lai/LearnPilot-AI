...
class TutorAgent:
    ...
    def ask(
        ...
        query = " ".join(part for part in (knowledge_point, question) if part)
        evidence = self.retriever.retrieve(query, resources, top_k=4)
        fallback = self._fallback(question, profile, evidence, history or [], knowledge_point)
        if isinstance(self.llm_client, TemplateLLMClient):
            return fallback

        try:
            generated = json.loads(self.llm_client.generate(self._prompt(question, profile, evidence, history or [])))
    ...
    def _prompt(
        ...
        return (
            ...
            f"学生问题：{question}\n"
            f"学生薄弱点：{profile.weak_points[:5]}\n"
            f"学习阶段：{profile.learning_stage}\n"
            f"对话历史：\n{history_text}\n"
            f"检索证据：\n{evidence_text}"
        )
